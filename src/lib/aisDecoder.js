import { isString, isObject } from 'cmd430-utils'
...
export class AISDecoder {
  ...
  constructor (input, session) {
    ...
    if (input.length === 0) {
      this.#error = 'Sentence is empty or spaces.'
      return
    } else if (!this.#validateChecksum(input)) {
      this.#error = 'Sentence checksum is invalid.'
      return
    }

    const nmea = input.split(',')
    if (nmea.length !== 7) {
      this.#error = 'Sentence contains invalid number of parts.'
      return
    }

    const command = nmea[0].substring(3,6)
    if (command !== 'VDM' && command !== 'VDO') {
      this.#error = 'Invalid message prefix.'
      return
    }
    ...
    this.#payload = new Buffer.from(nmea[5])
  ...
  GetNavStatus () {
    return AISDecoder.#NAV_STATUS[this.navstatus]
  }

  GetAisType () {
    return AISDecoder.#MSG_TYPE[this.aistype]
  }

  GetVesselType () {
    return AISDecoder.#VESSEL_TYPE[this.cargo]
  }
